...
class HypRelEncoder(nn.Module):
    ...
    """
    def __init__(self, encoder_type, graph_repr, config, entity_embs=None, rel_embs=None):
        ...
        self.n_bases = config['MODEL']['N_BASES']
        self.model_nm = config['MODEL_NAME'].lower()
        
        self.entity_embs = get_param((self.num_ent, self.emb_dim)) if entity_embs is None else entity_embs
        self.rel_embs    = get_param((self.num_rel * 2, self.emb_dim)) if rel_embs is None else rel_embs
        ...
        ### TODO: Base Embeddings??? 
        ### To easily integate see here - https://github.com/malllabiisc/CompGCN/commit/023cfb0b995f57558f14b20525266b205678c0e2
        #self.base_rel_embs  = get_param((self.p.n_bases, self.p.emb_dim))

        self.conv1 = CompGCNConv(self.encoder_type, self.emb_dim, self.gcn_dim, self.num_rel, act=self.act, params=config)
        self.conv2 = CompGCNConv(self.encoder_type, self.gcn_dim, self.emb_dim, self.num_rel, act=self.act, params=config) if self.n_layer == 2 else None
    ...
    def forward(self, sub, rel, quals):
        ...
        :return:
        """ 
        x, r = self.conv1(x=self.entity_embs, edge_index=self.edge_index, edge_type=self.edge_type, rel_embed=self.rel_embs, quals=self.quals)
        x = self.drop1(x)

        if self.n_layer == 2:
            x, r = self.conv2(x=x, edge_index=self.edge_index, edge_type=self.edge_type, rel_embed=r, quals=self.quals)
            x = self.drop2(x) 
