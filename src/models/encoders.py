...
class HypRelEncoder(nn.Module):
    ...
    Define the encoder
    """
    def __init__(self, encoder_type, graph_repr, config):
        ...
        self.conv2 = CompGCNConv(self.encoder_type, self.gcn_dim, self.emb_dim, self.num_rel, act=self.act, params=config)
    ...
    def forward(self, sub, rel, quals, entity_embs, rel_embs):
        """"
        Pass through encoder

        :return:
        """
        x, r = self.conv1(x=entity_embs, edge_index=self.edge_index, edge_type=self.edge_type, rel_embed=rel_embs, quals=self.quals)
        x = self.drop1(x)
