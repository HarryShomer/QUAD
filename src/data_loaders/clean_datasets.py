"""
Original source can be found here - https://github.com/migalkin/StarE/blob/master/data_loaders/clean_datasets.py
"""
...
def load_clean_wikipeople_statements(subtype, maxlen=17) -> Dict:
    ...
    """
    DIRNAME = os.path.join(DATA_DIR, "clean", "wikipeople")

    # Load raw shit
    with open(os.path.join(DIRNAME, 'train.txt'), 'r') as f:
        raw_trn = []
    ...
    with open(os.path.join(DIRNAME, 'valid.txt'), 'r') as f:
        raw_val = []
        for line in f.readlines():
...
def load_clean_jf17k_statements(subtype, maxlen=15) -> Dict:
    PARSED_DIR = os.path.join(DATA_DIR, "clean", "jf17k")

    training_statements = []
    test_statements = []

    with open(os.path.join(PARSED_DIR, 'train.txt'), 'r') as train_file, \
        open(os.path.join(PARSED_DIR, 'test.txt'), 'r') as test_file:
...
def load_clean_wd50k(name, subtype, maxlen=43) -> Dict:
    ...
    DIRNAME = os.path.join(DATA_DIR, "clean", name, subtype)
